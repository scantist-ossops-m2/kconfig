...
static QVarLengthArray<int, 8> asIntList(QByteArrayView string)
{
    ...
    while (next != -1) {
        next = string.indexOf(',', start);
        auto len = (next - start) < 0 ? string.size() - start : next - start;
        ret.push_back(string.sliced(start, len).toInt());
        start = next + 1;
    }
    return ret;
}
...
static QVarLengthArray<qreal, 8> asRealList(QByteArrayView string)
{
    ...
    while (next != -1) {
        next = string.indexOf(',', start);
        auto len = (next - start) < 0 ? string.size() - start : next - start;
        ret.push_back(string.sliced(start, len).toDouble());
        start = next + 1;
    }
    return ret;
}
